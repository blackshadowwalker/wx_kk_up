...
#define MAX_CNT   (16)
volatile int   nWriteCnt = 0;

typedef enum {
	ZLOG_LEVEL_DEBUG = 0x0001<<4, // 16,
	ZLOG_LEVEL_INFO = 0x0001<<5, //32,
	ZLOG_LEVEL_NOTICE = 0x0001<<6, // 64,
	ZLOG_LEVEL_WARN = 0x0001<<7, //128,
	ZLOG_LEVEL_ERROR = 0x0001<<8, //256,
	ZLOG_LEVEL_FATAL = 0x0001<<9 //512
} zlog_level; 

static char cLogBuff[MAX_CNT][2048];
...
int zlogwrite(int fd,char *data,int len, int level) {
  int numb = 0;
  int status;
  int nWritePos = 0;
  data[len]='\0';

  nWritePos = nWriteCnt%MAX_CNT;
  strcpy(&cLogBuff[nWritePos][0],data);
  nWriteCnt++;

  if( level &  LOG_EVEL )
	  PostMessage(zLogWndHandle,(WM_USER+40),&cLogBuff[nWritePos][0],NULL);
...
static int zlog_rule_output_static_file_single(zlog_rule_t * a_rule, zlog_thread_t * a_thread)
{
	...
	if (zlogwrite(a_rule->static_fd,
			zlog_buf_str(a_thread->msg_buf),
			zlog_buf_len(a_thread->msg_buf) ,a_thread->event->level) < 0) {
...
static int zlog_rule_output_static_file_rotate(zlog_rule_t * a_rule, zlog_thread_t * a_thread)
{
	...
	if (zlogwrite(a_rule->static_fd, zlog_buf_str(a_thread->msg_buf), len, a_thread->event->level) < 0) {
		zc_error("write fail, errno[%d]", errno);
		return -1;
	}
...
static int zlog_rule_output_dynamic_file_single(zlog_rule_t * a_rule, zlog_thread_t * a_thread)
{
	...
	fd = zlogopen(zlog_buf_str(a_thread->path_buf),
		a_rule->file_open_flags | O_WRONLY | O_APPEND | O_CREAT, a_rule->file_perms);
	if (fd < 0) {
	...
	if (zlogwrite(fd, zlog_buf_str(a_thread->msg_buf), zlog_buf_len(a_thread->msg_buf),a_thread->event->level) < 0) {
		zc_error("write fail, errno[%d]", errno);
		zlogclose(fd);
		return -1;
	}
...
static int zlog_rule_output_dynamic_file_rotate(zlog_rule_t * a_rule, zlog_thread_t * a_thread)
{
	...
	fd = zlogopen(path, a_rule->file_open_flags | O_WRONLY | O_APPEND | O_CREAT, a_rule->file_perms);
	if (fd < 0) {
		zc_error("open file[%s] fail, errno[%d]", zlog_buf_str(a_thread->path_buf), errno);
		return -1;
	}
	...
	len = zlog_buf_len(a_thread->msg_buf);
	if (zlogwrite(fd, zlog_buf_str(a_thread->msg_buf), len,a_thread->event->level) < 0) {
		zc_error("write fail, errno[%d]", errno);
		zlogclose(fd);
		return -1;
	}
...
long lStdOut = STDOUT_FILENO;
	...
	if (zlogwrite(lStdOut,
		zlog_buf_str(a_thread->msg_buf), zlog_buf_len(a_thread->msg_buf),a_thread->event->level) < 0) {
		zc_error("write fail, errno[%d]", errno);
		return -1;
	}
		...
		size_t fsync_period)
{
	...
	case '"' :
		...
		if (a_rule->dynamic_specs) {
			...
			a_rule->static_fd = zlogopen(a_rule->file_path,
