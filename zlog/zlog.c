...
zlog_conf_t *zlog_env_conf = NULL;
static pthread_key_t zlog_thread_key = NULL ;
static zc_arraylist_t *zlog_env_threads = NULL ;
static zc_hashtable_t *zlog_env_categories = NULL ;
static zc_hashtable_t *zlog_env_records = NULL ;
static zlog_category_t *zlog_default_category = NULL ;
static size_t zlog_env_reload_conf_count;
static int zlog_env_init_flag = -1;

#ifdef WIN32
 HWND  zLogWndHandle = 0;
 int	 LOG_EVEL = 0xFFFF;
#endif
/*******************************************************************************/
...
int dzlog_set_wnd_handle(HWND wndHandle, int logLevel)
{
	zLogWndHandle = wndHandle;
	LOG_EVEL = logLevel ;
}
/*******************************************************************************/
	...
	const char *format, ...)
{
	...
	if(zlog_default_category == NULL)
	{
		return ;
	}

	if (zlog_category_needless_level(zlog_default_category, level)) 
		return;
