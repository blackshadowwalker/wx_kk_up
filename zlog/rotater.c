...
#include <unixport/glob_unixem.h>
#include <glob.h>
#endif
...
static zlog_file_t *zlog_file_check_new(zlog_rotater_t * a_rotater, const char *path)
{
	...
	if (nscan == 0)
		nread = 0; /* if nothing is scaned, nread will be a random number */
...
static int zlog_rotater_roll_files(zlog_rotater_t * a_rotater)
{
	...
	int rc = 0;
	int nwrite = 0;
	char new_path[MAXLEN_PATH + 1] = {0};
	char cFmt[512] = {0};
	zlog_file_t *a_file = NULL;
	...
	for (i = zc_arraylist_len(a_rotater->files) - 1; i > -1; i--) {
		...
		if(((int)a_rotater->num_end_len - (int) a_rotater->num_start_len) > 1)
		{
				sprintf(cFmt,"%%.%ds%%0%dd%%s",(int) a_rotater->num_start_len,a_rotater->num_width);
			nwrite = sprintf(new_path,  cFmt,
				a_rotater->glob_path, 
				i + 1,
				a_rotater->glob_path + a_rotater->num_end_len);
		}
		...
		{
				sprintf(cFmt,"%%.%ds%%0%dd%",(int) a_rotater->num_start_len,a_rotater->num_width);
			nwrite = sprintf(new_path,  cFmt,
				a_rotater->glob_path, 
				i + 1);
		}
