...
Database::Database(const std::string& h,const std::string& u,const std::string& p,const std::string& d,const int& port,IError *e)
:host(h)
,user(u)
,password(p)
,database(d)
...
Database::Database(Mutex& m,const std::string& h,const std::string& u,const std::string& p,const std::string& d,const int& port,IError *e)
:host(h)
,user(u)
,password(p)
,database(d)
...
Database::OPENDB *Database::grabdb()
{
	...
	if (!odb)
	{
		...
		if (m_embedded)
		{
			if (!mysql_real_connect(&odb -> mysql,NULL,NULL,NULL,database.c_str(),netport,NULL,0) )
			{
		...
		{
			if (!mysql_real_connect(&odb -> mysql,host.c_str(),user.c_str(),password.c_str(),database.c_str(),netport,NULL,0) )
			{
