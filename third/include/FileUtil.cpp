...
list<char*>* FileUtil::ListFiles(char *baseDir, char *curDir, list<char*> &listResult, char *fileExt, int PathNameType, bool bAddDirectory, bool bListSub)
{
	...
	do
	{
		...
		//
		if(PathNameType==0)
			sprintf(szFindPath, "%s\\%s\\%s", baseDir, curDir, wfd.cFileName);//绝对目录
		else if(PathNameType==1 && ( curDir!=0 && lstrlen(curDir)>0 )){
			sprintf(szFindPath, "%s\\%s", curDir, wfd.cFileName);//相对目录
		}else
			sprintf(szFindPath, "%s",  wfd.cFileName);//文件名
		...
		{//是文件夹
			...
			if(bAddDirectory)
				listResult.push_back( szFindPath );//添加目录到list
				
			if(bListSub){ //列出子目录下的文件
