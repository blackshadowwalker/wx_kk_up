...
{	
...
#if __TF_ITS_BAYONET
	CString verData = "tfBayonet.dat";
#else
	CString verData = "tfEPolice.dat";
#endif
	...
	CString URL = UpdateServerURL + verData;
...
#if __TF_ITS_BAYONET 
	CString exeName = "tfBayonet.zip";
#else
	CString exeName = "tfEPolice.zip";
#endif
	...
	CString msg;
	msg.Format(_T("一个新版本的 %s (版本 %s )现在可用，继续升级吗?"), exeName,updateVersion);
	if (IDNO == MessageBox(GetActiveWindow(), msg, _T("新版本提示"), MB_YESNO|MB_ICONQUESTION))
	{
...
{
	char cTmpPath[MAX_PATH_LENGTH] = {0};

	int type = (WaitForReboot) ? MOVEFILE_DELAY_UNTIL_REBOOT : MOVEFILE_COPY_ALLOWED;

	const TCHAR *backup = _T("bakupdata");
	CString directory = executable.Left(executable.ReverseFind(_T('\\')));	
	CString backupFile = directory + _T('\\') + CString(backup);
	CString tmpZip = directory + _T('\\') + CString("tmp");


	//删除旧的版本的备份文件
	RemoveDir(backupFile);
	
	//新建立备份目录
	CreateMultipleDirectory((char*)(LPCSTR)backupFile);

	//备份当前的目录
	copydir((char*)(LPCSTR)directory,(char*)(LPCSTR)backupFile,"bakupdata");


	//创建zip 解压目录
	CreateMultipleDirectory((char*)(LPCSTR)tmpZip);

	GetCurrentDirectory(MAX_PATH_LENGTH,cTmpPath);
	SetCurrentDirectory(tmpZip);
	
	HZIP hz = OpenZip(update,0);
	ZIPENTRY ze; 
	GetZipItem(hz,-1,&ze);
	...
	movedir((char*)(LPCSTR)tmpZip,(char*)(LPCSTR)directory,"tmp");
	/*
